...
from app.schemas.recipe import RecipeOut, RecipeCreate, RecipeUpdate, RecipeWithCounts, RecipeDetailResponse
...
@router.get("/",status_code=status.HTTP_200_OK, response_model = List[RecipeWithCounts])
def fetch_recipe(db: Session = Depends(get_db)):
    return get_all_recipes(db)

@router.get("/{recipe_id}", status_code = status.HTTP_200_OK, response_model = RecipeDetailResponse)
